...
contract DiceGameFactory is Ownable{ 
  ...
  mapping (string => uint256) internal roomNameToIndex;
  ...
  function getGameId(string memory _roomName) public view returns(uint256) {
  ...
  function newGame() public {
    ...
    Game memory game = Game(players, 0, scores, bets, roomName, false, Status.BetsPending);
    games.push(game);
  ...
  function initialize(uint256 _gameId, string memory _roomName) private {
  ...
  function joinGame(uint256 _gameId) public notInGame(_gameId){
    Game storage game = games[_gameId];
